...
namespace bitcoinTradingFramework
{
	...
	struct Vector2
	{
		...
		public Vector2(int x, int y) : this((double)x, (double)y) {	}
		public double m_x;
		public double m_y;
	}
	...
	{
		#if WINDOWS
		const double kZoomScale = 0.05;
		const int kBorder = 5;

		Form m_debugRenderForm;
		int m_width;
		int m_height;

		const string kAskSeriesName = "Ask";
		const string kBidSeriesName = "Bid";
		const string kPriceSeriesName = "price";
		const string kChartAreaName = "area";
		
		const string kAskSeries = "ask";
		const string kBidSeries = "bid";
		//const string kDepthChartAreaName = "depth";

		const string kProfitChartAreaName = "profitarea";
		const string kProfitSeriesName = "ROI %";

		const string kDateFormat = "dd/MMM\nhh:mm";

		const int kNumCharts = 2;

		Chart m_chart;
		ChartArea m_area;
		double m_deltaScrollTotal;
		Vector2? m_mouseDown;
		HitTestResult m_mouseOverResult;

		//Chart m_depthChart;
		//ChartArea m_depthArea;

		Chart m_profitChart;
		ChartArea m_profitArea;

		List<decimal> m_bidData;
		List<decimal> m_askData;
		List<decimal> m_lastPrice;
		List<DateTime> m_times;

		List<decimal> m_tradeDistanceFromTrue;
		List<decimal> m_tradeVolume;

		List<decimal> m_profitPercentage;
		List<DateTime> m_profitTimeSeconds;

		Dictionary<DateTime, List<MarkerContainer>> m_markers;
		...
		public Rendering(int width, int height)
		{
			...
			Thread windowsThread = new Thread(WindowsThread);
			windowsThread.Start();
		}
		...
		public void AddDataPoint(decimal smaF, decimal smaS, decimal price, DateTime now, bool reformatGraph=true)
		{
			m_debugRenderForm.BeginInvoke( (Action)(() =>
			{
				...
				if (i > 0)
				{
					...
					foreach (KeyValuePair<DateTime, List<MarkerContainer>> kvp in m_markers.ToList())
					{
		...
		/// <param name="d"></param>
		/// <returns></returns>
		int FindIndexOfDate(DateTime d)
		{
		...
		/// <param name="d"></param>
		/// <returns></returns>
		int FindClosestDate(DateTime d)
		{
			int closestI = -1;
		...
		void ReformatGraphInternal()
		{
			if (m_askData.Count > 0)
			{
				...
				lock (m_markers)
				{
					foreach (KeyValuePair<DateTime, List<MarkerContainer>> kvp in m_markers)
					{
						...
						foreach (MarkerContainer c in kvp.Value)
						{
							if (index != -1)
							{
								ArrowAnnotation annotation = new ArrowAnnotation { ArrowSize = 3, ArrowStyle = System.Windows.Forms.DataVisualization.Charting.ArrowStyle.Simple };
								...
								bool allow = (!c.m_actioned && (DateTime.UtcNow - kvp.Key) < new TimeSpan(0, 10, 0)) || c.m_actioned;
								if (allow)
								{
				...
				decimal ymin = Math.Min(Math.Min(Math.Min(m_askData.Min(), m_bidData.Min()), m_lastPrice.Min()), markerMin);
				decimal ymax = Math.Max(Math.Max(Math.Max(m_askData.Max(), m_bidData.Max()), m_lastPrice.Max()), markerMax);
				decimal range = ymax - ymin;
		...
		void WindowsThread()
		{
			...
			m_debugRenderForm.SizeChanged += new EventHandler(OnWindowResized);
			...
			Legend smaSLegend = new Legend(kAskSeriesName);
			Legend smaFLegend = new Legend(kBidSeriesName);
			Legend priceLegend = new Legend(kPriceSeriesName);
		...
		/// <param name="y"></param>
		/// <returns></returns>
		PointF ToPixelsPos(ChartGraphics cg, double x, double y)
		{
			PointF p = new PointF((float)cg.GetPositionFromAxis(kChartAreaName, AxisName.X, x),
		...
		void OnPostPaintDepth(object sender, ChartPaintEventArgs e)
		{
		...
		/// <param name="x"></param>
		/// <param name="y"></param>
		void GetZoomRange(MouseEventArgs e, out Vector2 x, out Vector2 y, Vector2? startPos=null)
		{
			// Additional calculation in order to obtain pseudo
			// "positional zoom" feature

			Vector2 now = new Vector2(e.X, e.Y);

			double minXScale = (double)e.X / (double)m_chart.Width;
			double maxXScale = 1 - minXScale;
			double minYScale = (double)e.Y / (double)m_chart.Height;
		...
		/// <param name="sender"></param>
		/// <param name="e"></param>
		void RenderDebug(object sender, PaintEventArgs e)
		{
